...
class PNUPGrad(GramianWeightedAggregator):
    """
    :class:`~torchjd.aggregation._aggregator_bases.Aggregator` Probabilistic Normalized UPGrad
    ...
    def __init__(
        ...
        super().__init__(
            _PNUPGradWrapper(weighting, prob=prob, norm_eps=norm_eps, reg_eps=reg_eps, solver=solver)
        )
    ...
    def __repr__(self) -> str:
        return (
            f"{self.__class__.__name__}(pref_vector={repr(self._pref_vector)}, prob={self._prob}, norm_eps="
            f"{self._norm_eps}, reg_eps={self._reg_eps}, solver={repr(self._solver)})"
        )

    def __str__(self) -> str:
        return f"PNUPGrad{pref_vector_to_str_suffix(self._pref_vector)}"


class _PNUPGradWrapper(Weighting[PSDMatrix]):
    """
    ...
    :param prob: The probability of using the normalized gramian.
    :param norm_eps: A small value to avoid division by zero when normalizing.
    :param reg_eps: A small value to add to the diagonal of the gramian of the matrix. Due to
        numerical errors when computing the gramian, it might not exactly be positive definite.
        This issue can make the optimization fail. Adding ``reg_eps`` to the diagonal of the gramian
