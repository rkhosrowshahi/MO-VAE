"""
Training script for PixelCNN Prior on VQ-VAE and GG-VQ-VAE (single-level) discrete codes.

VQ-VAE / GG-VQ-VAE checkpoint can be loaded from:
  - wandb run (--wandb_id) - downloads via wandb API
  - Local path (--vqvae_checkpoint) - use existing file
...
Usage:
    # From wandb run
    python train_prior_vqvae.py --wandb_id c3hamj3d --wandb_project mo-vae --wandb_entity rasa_research \
                                --dataset cifar10 --epochs 100 --use_wandb

    # From local checkpoint
    python train_prior_vqvae.py --vqvae_checkpoint path/to/vqvae.pth --dataset cifar10 --epochs 100
"""
...
from models import get_network, VQVAE, PixelCNN
...
def parse_args():
    ...
    # Model paths: either wandb_id (download) OR vqvae_checkpoint (local)
    parser.add_argument('--wandb_id', type=str, default=None,
                        help='WandB run ID (e.g. c3hamj3d) to download VQ-VAE checkpoint from')
    parser.add_argument('--vqvae_checkpoint', type=str, default=None,
                        help='Local path to pre-trained VQ-VAE checkpoint (overrides wandb_id if set)')
...
def download_checkpoint_from_wandb(wandb_id: str, entity: str, project: str, download_dir: str) -> str:
    """
    Download VQ-VAE checkpoint from a wandb run.
    Tries: 1) run files (*.pth), 2) final_model artifact.
    Returns path to downloaded .pth file.
    """
    api = wandb.Api()
    ...
    # 1. Try run files (prefer final_checkpoint, skip prior checkpoints)
...
def load_vqvae(checkpoint_path, device):
    """Load pre-trained VQ-VAE or GG-VQ-VAE (single-level) from checkpoint."""
    ...
    if 'vq_layer.embedding.weight' not in model_state:
    ...
    if ckpt_args is not None:
        ...
        try:
            ...
            print(f"get_network failed ({e}), falling back to VQVAE")
            args = None
    else:
        args = None
    ...
    print(f"Loaded VQ-VAE: {num_embeddings} embeddings, dim={embedding_dim}")
    print(f"Latent size: {model.latent_spatial_dim}×{model.latent_spatial_dim}")
    
    return model, ckpt_args


def train_epoch_images(prior, vqvae, dataloader, optimizer, device, epoch):
    ...
    for images, _ in pbar:
        ...
        with torch.no_grad():
            z = vqvae.get_code_indices(images)
...
def train_epoch_codes(prior, dataloader, optimizer, device, num_embeddings, epoch):
    """Train one epoch from pre-extracted codes (LMDB)."""
    prior.train()
    loss_meter = AverageMeter()
    pbar = tqdm(dataloader, desc=f'Epoch {epoch}')
...
def generate_samples(prior, vqvae, num_samples, device, temperature=1.0):
    """Generate samples using prior + VQ-VAE decoder."""
    prior.eval()
    vqvae.eval()
...
def main():
    ...
    if args.vqvae_checkpoint is None and args.wandb_id is None:
        raise ValueError("Provide either --vqvae_checkpoint (local path) or --wandb_id (wandb run ID)")
    ...
    # Resolve VQ-VAE checkpoint path
    ...
    if checkpoint_path is None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            dest = checkpoints_dir / 'downloaded_vqvae.pth'
            shutil.copy(downloaded, dest)
            checkpoint_path = str(dest)
    
    print(f"Loading VQ-VAE from {checkpoint_path}")
    vqvae, ckpt_args = load_vqvae(checkpoint_path, device)
    
    # Dataset & normalizer from checkpoint if available
    ...
    # LMDB extraction (use checkpoint arch so LMDB path matches model type)
    arch = getattr(ckpt_args, 'arch', 'vq_vae') if ckpt_args else 'vq_vae'
    ...
    codes_dataset, use_lmdb = get_or_extract_codes_lmdb(
        vqvae, train_loader, device, save_root,
    ...
    # W&B init (do not resume yet; we'll do that after training if wandb_id)
    ...
    for epoch in range(1, args.epochs + 1):
        if use_codes:
            metrics = train_epoch_codes(
                prior, train_data, optimizer, device,
                vqvae.num_embeddings, epoch,
            )
        else:
            metrics = train_epoch_images(prior, vqvae, train_data, optimizer, device, epoch)
        
        scheduler.step()
        print(f"Epoch {epoch}/{args.epochs} - Loss: {metrics['loss']:.4f}")
        
        if args.use_wandb and args.wandb_id is None:
            wandb.log({'epoch': epoch, 'train/loss': metrics['loss']})
        ...
        if epoch % args.sample_every == 0 or epoch == args.epochs:
            samples = generate_samples(prior, vqvae, args.num_samples, device, args.temperature)
            sample_path = samples_dir / f'samples_epoch_{epoch:04d}.png'
            use_tanh = hasattr(vqvae, 'recons_activation') and vqvae.recons_activation is not None
            if use_tanh and type(vqvae.recons_activation).__name__ == 'Tanh':
    ...
    # Generate final samples for wandb upload
    num_vis = min(64, args.num_samples)
    final_samples = generate_samples(prior, vqvae, num_vis, device, args.temperature)
    ...
    # Update wandb run if we have wandb_id
    if args.use_wandb and args.wandb_id:
        ...
        gen_metrics = evaluate_generative_metrics(
            vqvae, test_loader, device, eval_args, prior=prior
        )
        ...
        # Replace final/* generative metrics
        run.summary.update({
